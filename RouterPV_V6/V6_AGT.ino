...
float safetyMargin_watts = 18;  // <<<------ increase for more export
//+++++++++++++++++++++++++++++++++++++++++++++++++
int voie2 = 0;
...
{ 
  ...
  POWERCAL = 0.123;   //tigous : 0.18 org 0.12 à ajuster pour faire coincider la puissance vraie avec le realPwer.
  ...
  // Détection de la polarité de l'alternance
	...
	if (polarityNow == POSITIVE)
	{
      ...
      cycleCount++; // incrément Nb de périodes  
        ...
        */
        /*if (energyInBucket < minpowerbucketforfire && realEnergy < Minpowerroutable/2 && realEnergy >0) { //si le sceau est sous le seuil de routage et que l'énergie mesurée est inférieure au min soutirable par la charge mais positive on remet le sceau à 0
          energyInBucket = 0;
        } else {*/
          energyInBucket += realEnergy;  
       // }
        ...
        if (energyInBucket<(float)capacityOfEnergyBucket*0.35){
          energyInBucket -= ((safetyMargin_watts*1.5) / cyclesPerSecond);
          // energyInBucket += realEnergy/2;
        }else if (energyInBucket>(float)capacityOfEnergyBucket*0.47){
...
//       }
		...
		//		if (firingDelayInMicros > 8000) { firingDelayInMicros = halfsin;} // never fire
...
// Préparation du message  
      if((cycleCount >= 150)) { // display once per second
        ...
        message = "EIB>" + (String)(sumenergyInBucket/50); 
...
// pilotage de la voie Wifi et envoi du message
     if((cycleCount >= 150)) { // display once per second
        if (sumenergyInBucket/50 == capacityOfEnergyBucket){
          if (voie2 == 0) {
            voie2=20;
          } else if (sumrealPower/50 > 200) {
            voie2+=20;
          }
          else if (sumrealPower/50 > 100) {
            voie2+=5;
          }  else if (sumrealPower/50< safetyMargin_watts) {
            voie2-=1;
          } else if (sumrealPower/50> safetyMargin_watts*2){
            voie2+=1;
          } 
        /*} else if ( sumenergyInBucket/50 > capacityOfEnergyBucket*0.9 ) {
          voie2-=1;
        }else if ( sumenergyInBucket/50 > capacityOfEnergyBucket*0.5 ) {
          voie2-=5;
        }else if (voie2>10 ) {
          voie2-=10;
        }*/
        } else if ( sumrealPower/50 < -1000 ) {
          voie2=0;
        }else if ( sumrealPower/50 < -100 ) {
          voie2-=10;
        } else {
